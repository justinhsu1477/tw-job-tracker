...
- **LLM 語意匹配** — 關鍵字粗篩後，AI 對 top 15 做語意重排，抓住非標準技能描述
...
3. 關鍵字評分 + 薪資標準化
4. **LLM 語意重排** — AI 對 top 15 做語意匹配，抓住「跨部門協作 ≈ 專案管理」等非標準描述
5. 寫入 Notion Job Tracker Database
6. **AI 公司研究** — 搜尋產業、規模、PTT/Glassdoor 評價、薪資參考
7. 等你選擇後產生求職信
8. **面試追蹤** — 更新面試階段、日期、筆記
...
```
第一階段（Python，免費，瞬間）
  關鍵字匹配 + 熟練度加權 → 粗篩所有職缺 → keyword_score

第二階段（Claude LLM，~$0.01/次，30 秒）
  對 top 15 做語意匹配 → 抓住非標準技能描述 → semantic_score

最終分數 = keyword_score × 40% + semantic_score × 60%
```

語意匹配能解決關鍵字比對的盲點：
- ❌ 關鍵字：「跨部門協作經驗」vs「專案管理」→ 0 分
- ✅ 語意：兩者高度相關 → 給分
- ❌ 關鍵字：「具備系統設計能力」vs「架構經驗」→ 0 分
- ✅ 語意：同義 → 給分
